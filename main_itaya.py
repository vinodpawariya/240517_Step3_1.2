...
from search_property_itaya import search_property
...
if st.sidebar.button("検索"):
    df_search_property = search_property(rent_price_min, rent_price_max, add_fee_included, reikin, teiki, area_min_value, area_max_value,walk_min_value, walk_max_value, years_min_value, years_max_value, madori)
    # セッション状態に保存
    ...
    for index, row in st.session_state.df_search_property.iterrows():
        folium.Marker(
            location=[row['latitude'], row['longitude']],  # マーカーの座標
            popup=row['名称']  # マーカーをクリックしたときに表示されるポップアップ
        ).add_to(mymap)
